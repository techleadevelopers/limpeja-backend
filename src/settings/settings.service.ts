...
export class SettingsService {
  ...
  constructor(
    ...
    this.ttlSeconds = parseInt(this.config.get<string>('SETTINGS_TTL_SECONDS') || '2592000', 10); // 30d
    this.historyTtlSeconds = parseInt(this.config.get<string>('SETTINGS_HISTORY_TTL_SECONDS') || '31536000', 10); // 365d
  }
  ...
  async getSlaSettings(): Promise<SlaSettings> {
    ...
    const disputeDefaults: DisputeSlaConfig = {
      urgentHours: this.coerceInt(this.config.get('DISPUTE_SLA_URGENT_HOURS'), 4),
      highHours: this.coerceInt(this.config.get('DISPUTE_SLA_HIGH_HOURS'), 8),
      mediumHours: this.coerceInt(this.config.get('DISPUTE_SLA_MEDIUM_HOURS'), 24),
    ...
    const disputes: DisputeSlaConfig = {
      urgentHours: await this.getNumber(`${this.baseKey}:dispute:urgent_hours`, disputeDefaults.urgentHours),
      highHours: await this.getNumber(`${this.baseKey}:dispute:high_hours`, disputeDefaults.highHours),
      mediumHours: await this.getNumber(`${this.baseKey}:dispute:medium_hours`, disputeDefaults.mediumHours),
      lowHours: await this.getNumber(`${this.baseKey}:dispute:low_hours`, disputeDefaults.lowHours),
    };

    const support: SupportSlaConfig = {
      PAYMENT: await this.getNumber(`${this.baseKey}:support:PAYMENT_hours`, supportDefaults.PAYMENT),
      QUALITY: await this.getNumber(`${this.baseKey}:support:QUALITY_hours`, supportDefaults.QUALITY),
      APP: await this.getNumber(`${this.baseKey}:support:APP_hours`, supportDefaults.APP),
      OTHER: await this.getNumber(`${this.baseKey}:support:OTHER_hours`, supportDefaults.OTHER),
    };

    return { disputes, support };
  }

  async updateSlaSettings(partial: Partial<SlaSettings>, actorUserId?: string): Promise<SlaSettings> {
    ...
    const validateHours = (n: unknown, field: string) => {
      ...
      if (!Number.isFinite(v) || v <= 0 || v > 168) {
        throw new BadRequestException(`Invalid hours for ${field}. Expected 1..168.`);
      }
      return Math.floor(v);
    };
    ...
    if (partial.disputes) {
      if (partial.disputes.urgentHours != null) {
        next.disputes.urgentHours = validateHours(partial.disputes.urgentHours, 'disputes.urgentHours');
      }
      if (partial.disputes.highHours != null) {
        next.disputes.highHours = validateHours(partial.disputes.highHours, 'disputes.highHours');
      }
      if (partial.disputes.mediumHours != null) {
        next.disputes.mediumHours = validateHours(partial.disputes.mediumHours, 'disputes.mediumHours');
      }
      if (partial.disputes.lowHours != null) {
        next.disputes.lowHours = validateHours(partial.disputes.lowHours, 'disputes.lowHours');
      }
    }
    ...
    if (partial.support) {
      for (const key of ['PAYMENT', 'QUALITY', 'APP', 'OTHER'] as const) {
        ...
        if (incoming != null) {
          (next.support as any)[key] = validateHours(incoming, `support.${key}`);
        }
      }
    }

    // Persist to cache (Redis)
    await this.setNumber(`${this.baseKey}:dispute:urgent_hours`, next.disputes.urgentHours);
    await this.setNumber(`${this.baseKey}:dispute:high_hours`, next.disputes.highHours);
    await this.setNumber(`${this.baseKey}:dispute:medium_hours`, next.disputes.mediumHours);
    await this.setNumber(`${this.baseKey}:dispute:low_hours`, next.disputes.lowHours);

    await this.setNumber(`${this.baseKey}:support:PAYMENT_hours`, next.support.PAYMENT);
    await this.setNumber(`${this.baseKey}:support:QUALITY_hours`, next.support.QUALITY);
    await this.setNumber(`${this.baseKey}:support:APP_hours`, next.support.APP);
    await this.setNumber(`${this.baseKey}:support:OTHER_hours`, next.support.OTHER);
    ...
    try {
      ...
      const history = (await this.cache.get<SlaAuditEvent[]>(this.historyKey)) || [];
  ...
  async getSlaHistory(limit = 50, cursor = 0): Promise<{ items: SlaAuditEvent[]; nextCursor: number | null }> {
    ...
    const end = Math.min(history.length, start + Math.max(1, Math.min(200, limit)));
  ...
  async getGeneralSettings(): Promise<GeneralSettings> {
    const defaults: GeneralSettings = {
      commissionRatePercent: this.coerceInt(this.config.get('DEFAULT_COMMISSION_RATE_PERCENT'), 15),
    };
    const stored = await this.cache.get<GeneralSettings>(this.generalKey);
    return stored ? { ...defaults, ...stored } : defaults;
  }

  async updateGeneralSettings(partial: Partial<GeneralSettings>, actorUserId?: string): Promise<GeneralSettings> {
    ...
    try {
      ...
      const history = (await this.cache.get<GeneralAuditEvent[]>(this.generalHistoryKey)) || [];
      history.unshift(event);
      while (history.length > 500) history.pop();
      await this.cache.set(this.generalHistoryKey, history, this.historyTtlSeconds);
    } catch (e) {
      this.logger.warn(`Failed to append General audit: ${(e as Error)?.message}`);
    }

    return next;
  }

  async getGeneralHistory(limit = 50, cursor = 0): Promise<{ items: GeneralAuditEvent[]; nextCursor: number | null }> {
    const history = (await this.cache.get<GeneralAuditEvent[]>(this.generalHistoryKey)) || [];
    const start = Math.max(0, cursor | 0);
    const end = Math.min(history.length, start + Math.max(1, Math.min(200, limit)));
    const items = history.slice(start, end);
  ...
  async appendPricingAudit(event: PricingAuditEvent): Promise<void> {
    const history = (await this.cache.get<PricingAuditEvent[]>(this.pricingHistoryKey)) || [];
    history.unshift(event);
    while (history.length > 1000) history.pop();
    await this.cache.set(this.pricingHistoryKey, history, this.historyTtlSeconds);
  }

  async getPricingHistory(limit = 50, cursor = 0): Promise<{ items: PricingAuditEvent[]; nextCursor: number | null }> {
    const history = (await this.cache.get<PricingAuditEvent[]>(this.pricingHistoryKey)) || [];
    const start = Math.max(0, cursor | 0);
    const end = Math.min(history.length, start + Math.max(1, Math.min(200, limit)));
    const items = history.slice(start, end);
