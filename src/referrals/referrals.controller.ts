...
import { Controller, Post, Body, UseGuards, Req, Get, Param, NotFoundException, Logger } from '@nestjs/common';
...
export class ReferralsController {
  ...
  @ApiResponse({ status: 201, description: 'Indicação registrada com sucesso.', type: ReferralEntity })
  @ApiResponse({ status: 400, description: 'Dados de indicação inválidos.' })
  @ApiResponse({ status: 409, description: 'Indicação já existe.' })
  async createReferral(@Req() req, @Body() createReferralDto: CreateReferralDto) {
    this.logger.log(`[ReferralsController] createReferral: Registrando indicação de ${req.user.userId} para ${createReferralDto.referredUserId}`);
    // Garante que o referrerUserId do DTO é o usuário autenticado
    createReferralDto.referrerUserId = req.user.userId;
    const referral = await this.referralsService.createReferral(createReferralDto);
    return referral;
  }
  ...
  @ApiResponse({ status: 200, description: 'Lista de indicações feitas pelo usuário.', type: [ReferralEntity] })
  async getMyReferrals(@Req() req) {
    this.logger.log(`[ReferralsController] getMyReferrals: Buscando indicações para userId: ${req.user.userId}`);
    const referrals = await this.referralsService.findReferralsByReferrer(req.user.userId);
    return referrals;
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Obter indicação por ID (apenas para administradores)' })
  @ApiResponse({ status: 200, description: 'Detalhes da indicação.', type: ReferralEntity })
  @ApiResponse({ status: 404, description: 'Indicação não encontrada.' })
  async getReferralById(@Param('id') id: string) {
    this.logger.log(`[ReferralsController] getReferralById: Buscando indicação por ID: ${id}`);
