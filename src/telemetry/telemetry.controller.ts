...
export class TelemetryController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Forçar logout de um usuário identificado pela telemetria.' })
  @ApiResponse({
    status: 200,
  ...
  async forceLogout(@Param('userId') userId: string): Promise<{ message: string; userId: string }> {
    this.logger.warn(`[TelemetryController] forceLogout: usuário ${userId} marcado para logout forçado`);
    await this.telemetryService.markForceLogout(userId);
    return {
      message: 'Usuário marcado para logout forçado. A sessão será bloqueada nos próximos segundos.',
      userId,
    };
  }
}
