...
class TransactionDto {
  ...
  @ApiProperty({ description: 'Tipo da transação (pagamento, saque, comissão)', example: 'PAYMENT' })
...
export class EarningsResponseDto {
  @ApiProperty({ description: 'Total de ganhos acumulados do provedor', example: 5000.00 })
  totalEarnings: number;

  @ApiProperty({ description: 'Valor disponível para saque', example: 1200.50 })
  ...
  @ApiPropertyOptional({
    description: 'Breakdown dos ganhos por período (ex: mensal, semanal) para gráficos',
...
export class WithdrawalRequestDto {
  @ApiProperty({ description: 'Valor a ser sacado', example: 100.00 })
  amount: number;

  @ApiPropertyOptional({ description: 'Informações da conta bancária para saque (ex: chave PIX)', example: 'minhachavepix@email.com' })
...
export class WithdrawalResponseDto {
  ...
  @ApiProperty({ description: 'Mensagem de retorno sobre a solicitação', example: 'Solicitação de saque de R$ 100.00 enviada com sucesso. Processamento em até 2 dias úteis.' })
  message: string;

  @ApiPropertyOptional({ description: 'ID da transação de saque gerada', example: 'withdrawal_abc123' })
