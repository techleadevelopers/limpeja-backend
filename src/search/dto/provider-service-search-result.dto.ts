...
export class ProviderServiceSearchResultDto {
  @ApiProperty({ type: () => ProviderDetailsDto, description: 'Detalhes do provedor (com sinais premium opcionais)' })
  ...
  @ApiPropertyOptional({ description: 'Status de verificação (para selo nos cards)', enum: ['APPROVED', 'PENDING', 'REJECTED'] })
  @IsOptional()
  @IsString()
  verificationStatus?: string;

  @ApiPropertyOptional({ description: 'Taxa de aceitação (para métricas mini)', example: 90 })
  @IsOptional()
  @IsNumber()
  acceptanceRate?: number;

  @ApiPropertyOptional({ description: 'Tempo médio de resposta (para métricas mini)', example: 15 })
  @IsOptional()
  @IsNumber()
  averageResponseTime?: number;

  @ApiPropertyOptional({ description: 'Próximo horário disponível (para chip nos cards)', example: { date: '2025-09-29', time: '09:00' } })
  @IsOptional()
  nextAvailable?: { date: string; time: string };

  @ApiPropertyOptional({ description: 'Badges do provedor (opcional)', type: [String], example: ['TOP_RATED'] })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
  badges?: string[];
