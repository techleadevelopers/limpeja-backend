...
export class ProviderServicesController {
  ...
  private async validateProviderOwnership(
    ...
    if (!provider || provider.id !== providerId) {
      throw new ForbiddenException(
        'Você não tem permissão para gerenciar os serviços deste provedor.',
      );
    }
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Adicionar um novo serviço oferecido por um provedor',
  })
  // CORREÇÃO: Use ProviderServiceEntity
  @ApiResponse({
    status: 201,
    description: 'Serviço oferecido adicionado com sucesso.',
  ...
  async create(
    ...
    // CORREÇÃO: Retorna ProviderServiceEntity
    await this.validateProviderOwnership(req, providerId);
    const createdService = await this.providerServicesService.create(
      providerId,
  ...
  @Get()
  @ApiOperation({
    summary: 'Listar todos os serviços oferecidos por um provedor',
  })
  // CORREÇÃO: Use [ProviderServiceEntity]
  @ApiResponse({
    status: 200,
  ...
  async findAll(
    ...
    // CORREÇÃO: Retorna ProviderServiceEntity[]
    const services =
      await this.providerServicesService.findAllByProviderId(providerId);
  ...
  @ApiResponse({
    status: 200,
    description: 'Serviço oferecido atualizado com sucesso.',
    type: ProviderServiceEntity,
  })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @ApiResponse({ status: 403, description: 'Acesso proibido.' })
  ...
  async update(
    ...
    // CORREÇÃO: Retorna ProviderServiceEntity
    await this.validateProviderOwnership(req, providerId);
    const updatedService = await this.providerServicesService.update(
      id,
      providerId,
  ...
  @ApiResponse({
    status: 204,
    description: 'Serviço oferecido removido com sucesso.',
  })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @ApiResponse({ status: 403, description: 'Acesso proibido.' })
  @ApiResponse({
