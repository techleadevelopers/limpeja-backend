...
export class GeocodingService {
  ...
  constructor(private configService: ConfigService) {
    this.geocodingProvider = this.configService.get<string>('geocoding.provider');
    this.googleMapsApiKey = this.configService.get<string>('geocoding.googleMapsApiKey');
    this.openStreetMapNominatimUrl = this.configService.get<string>('geocoding.openStreetMapNominatimUrl');

    if (!this.geocodingProvider) {
      this.logger.warn('Nenhum provedor de geocodificação configurado. O serviço de geocodificação estará em modo de simulação.');
    } else {
      this.logger.log(`Provedor de geocodificação configurado: ${this.geocodingProvider}`);
    }
  }
  ...
  async geocodeAddress(address: string): Promise<GeoCoordinates | null> {
    this.logger.log(`Geocodificando endereço: "${address}" usando provedor: ${this.geocodingProvider || 'SIMULADO'}`);

    try {
      switch (this.geocodingProvider) {
        case 'GOOGLE_MAPS':
          if (!this.googleMapsApiKey) {
            this.logger.error('GOOGLE_MAPS_API_KEY não configurada para o provedor Google Maps.');
          ...
          // }
          this.logger.warn('Integração real com Google Maps API não implementada. Usando simulação.');
          return this.simulateGeocoding(address);

        case 'OPENSTREETMAP':
          if (!this.openStreetMapNominatimUrl) {
            this.logger.error('OPENSTREETMAP_NOMINATIM_URL não configurada para o provedor OpenStreetMap.');
            return this.simulateGeocoding(address);
          }
          ...
          // }
          this.logger.warn('Integração real com OpenStreetMap Nominatim API não implementada. Usando simulação.');
          return this.simulateGeocoding(address);
  ...
  private async simulateGeocoding(address: string): Promise<GeoCoordinates | null> {
