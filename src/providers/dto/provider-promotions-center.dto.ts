...
class ProviderCouponDto {
  @ApiProperty({ description: 'Código público do cupom', example: 'PROV-10OFF' })
  code: string;

  @ApiProperty({ description: 'Descrição da promoção', example: '10% off exclusivo para você' })
  description?: string;
  ...
  @ApiProperty({ description: 'Data inicial de validade (ISO)', example: '2025-12-01T00:00:00Z' })
  validFrom: string;

  @ApiProperty({ description: 'Data final de validade (ISO)', example: '2025-12-31T23:59:59Z' })
  validUntil: string;

  @ApiProperty({ description: 'Status público do cupom', example: CouponStatus.ACTIVE })
  status: CouponStatus;
}

class LoyaltyRewardDto {
  @ApiProperty({ description: 'ID interno da recompensa', example: 'reward-abc' })
  rewardId: string;

  @ApiProperty({ description: 'Texto descritivo da recompensa', example: 'Cupom de R$ 15' })
  description: string;
...
class LoyaltySummaryDto {
  @ApiProperty({ description: 'Saldo atual de pontos do provider', example: 1200 })
...
export class ProviderPromotionsCenterViewDto {
  @ApiProperty({
    description: 'Promoções/cupons ativos emitidos com target SPECIFIC_PROVIDER',
    type: [ProviderCouponDto],
  })
  coupons: ProviderCouponDto[];

  @ApiProperty({
    description: 'Missões relevantes para o provider com progresso e recompensa',
    type: [MissionViewDto],
  })
