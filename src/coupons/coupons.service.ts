...
export class CouponsService {
  ...
  async create(createCouponDto: CreateCouponDto & { issuedToUserId?: string; issuedBy?: string; maxDiscount?: number }) {
    const {
      code,
      validFrom,
      validUntil,
    ...
    const createdCoupon = await this.prisma.coupon.create({
      data: {
        ...
        status: isActive === false ? CouponStatus.INACTIVE : CouponStatus.ACTIVE, // <<-- FIXED: Use CouponStatus enum
    ...
    this.logger.log(`[CouponsService] Cupom ${createdCoupon.code} criado. IssuedBy: ${createdCoupon.issuedBy}, IssuedTo: ${createdCoupon.issuedToUserId}`);
    // Telemetria: coupon_created
    this.logger.log(`[TELEMETRY] coupon_created: { couponId: ${createdCoupon.id}, code: ${createdCoupon.code}, issuedBy: ${createdCoupon.issuedBy}, issuedToUserId: ${createdCoupon.issuedToUserId} }`);
  ...
  async update(id: string, dto: UpdateCouponDto) {
    ...
    this.logger.log(`[TELEMETRY] coupon_updated: { couponId: ${updatedCoupon.id}, code: ${updatedCoupon.code} }`);
  ...
  async applyCoupon(
    ...
    const coupon = await this.prisma.coupon.findUnique({ where: { code: code.toUpperCase() } });
    if (!coupon) {
      this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} não encontrado.`);
      return { discountAmount: 0, newTotalPrice: originalPrice, message: 'Cupom inválido.' };
    }

    // Telemetria: coupon_viewed (ou attempted_apply)
    this.logger.log(`[TELEMETRY] coupon_viewed: { couponId: ${coupon.id}, code: ${coupon.code}, userId: ${userId} }`);

    const now = new Date();
    if (coupon.validFrom > now || coupon.validUntil < now) {
      this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} expirado ou ainda não ativo.`);
    ...
    if (coupon.status !== CouponStatus.ACTIVE) { // <<-- FIXED: Use CouponStatus enum
    ...
    if (coupon.issuedToUserId && coupon.issuedToUserId !== userId) {
      this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} não é para o usuário ${userId}. Foi emitido para ${coupon.issuedToUserId}.`);
      return { discountAmount: 0, newTotalPrice: originalPrice, message: 'Este cupom não é seu.' };
    }
    ...
    if (coupon.firstBookingOnly) {
      ...
      if (!client) {
        this.logger.warn(`[CouponsService] applyCoupon: Cliente não encontrado para userId ${userId} ao aplicar cupom firstBookingOnly.`);
        return { discountAmount: 0, newTotalPrice: originalPrice, message: 'Cliente não encontrado.' };
      }
      ...
      if (client.completedBookingsCount > 0) {
        this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} é exclusivo para novos clientes, mas ${userId} já tem ${client.completedBookingsCount} bookings concluídos.`);
    ...
    if (coupon.target === CouponTarget.NEW_CLIENTS) {
      ...
      if (client.completedBookingsCount > 0) {
        this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} (target NEW_CLIENTS) não aplicável a cliente existente ${userId}.`);
        return { discountAmount: 0, newTotalPrice: originalPrice, message: 'Cupom exclusivo para novos clientes.' };
      }
      ...
      if (!bookingData.providerServiceId || bookingData.providerServiceId !== coupon.targetId) {
        this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} não aplicável a este serviço. Esperado: ${coupon.targetId}, Recebido: ${bookingData.providerServiceId}.`);
      ...
      if (!bookingData.providerId || bookingData.providerId !== coupon.targetId) {
        this.logger.warn(`[CouponsService] applyCoupon: Cupom ${code} não aplicável a este provedor. Esperado: ${coupon.targetId}, Recebido: ${bookingData.providerId}.`);
    ...
    if (coupon.maxDiscount && discountAmount > Number(coupon.maxDiscount)) {
      this.logger.log(`[CouponsService] applyCoupon: Desconto de ${discountAmount.toFixed(2)} limitado pelo maxDiscount do cupom (${Number(coupon.maxDiscount).toFixed(2)}).`);
    ...
    this.logger.log(`[CouponsService] applyCoupon: Cupom ${code} aplicado com sucesso. Desconto: ${discountAmount.toFixed(2)}, Novo Preço: ${newTotalPrice.toFixed(2)}.`);
    // Telemetria: coupon_applied
    this.logger.log(`[TELEMETRY] coupon_applied: { couponId: ${coupon.id}, code: ${coupon.code}, userId: ${userId}, discountAmount: ${discountAmount.toFixed(2)}, newTotalPrice: ${newTotalPrice.toFixed(2)} }`);
  ...
  async markCouponAsUsed(couponId: string) {
    ...
    if (!coupon) {
      this.logger.warn(`[CouponsService] markCouponAsUsed: Cupom ${couponId} não encontrado.`);
      return;
    }
    ...
    if (updatedCoupon.maxUses && updatedCoupon.usesCount >= updatedCoupon.maxUses) {
      await this.prisma.coupon.update({
        where: { id: couponId },
        data: { status: CouponStatus.USED_UP }, // <<-- FIXED: Use CouponStatus enum
      });
      this.logger.log(`[CouponsService] markCouponAsUsed: Cupom ${couponId} atingiu o limite de usos e foi marcado como USED_UP.`);
    }

    this.logger.log(`[CouponsService] markCouponAsUsed: Cupom ${couponId} uso registrado. Total de usos: ${updatedCoupon.usesCount}.`);
    // Telemetria: coupon_used
    this.logger.log(`[TELEMETRY] coupon_used: { couponId: ${coupon.id}, code: ${coupon.code} }`);
  }
  ...
  async issueCouponFromMission(params: {
    ...
    this.logger.log(`[CouponsService] Cupom de missão ${created.code} emitido para ${userId}.`);
    // Telemetria: mission_coupon_issued
    this.logger.log(`[TELEMETRY] mission_coupon_issued: { couponId: ${created.id}, userId: ${userId}, missionId: ${mission.id} }`);
  ...
  async issueReturnCoupon(userId: string, bookingId: string): Promise<any> {
    ...
    this.logger.log(`[CouponsService] Cupom de retorno ${coupon.code} emitido para ${userId} após booking ${bookingId}.`);
    // Telemetria: return_coupon_issued
    this.logger.log(`[TELEMETRY] return_coupon_issued: { couponId: ${coupon.id}, userId: ${userId}, bookingId: ${bookingId} }`);
    return coupon;
  }
   ...
   */
  async issueReferralReferredCoupon(userId: string, referralId: string): Promise<any> {
    const now = new Date();
    const validUntil = new Date(now);
    ...
    this.logger.log(`[CouponsService] Cupom de indicação (indicado) ${coupon.code} emitido para ${userId} (referral ${referralId}).`);
    // Telemetria: referral_referred_coupon_issued
    this.logger.log(`[TELEMETRY] referral_referred_coupon_issued: { couponId: ${coupon.id}, userId: ${userId}, referralId: ${referralId} }`);
    return coupon;
  }
   ...
   */
  async issueReferralReferrerCoupon(userId: string, referralId: string): Promise<any> {
    const now = new Date();
    const validUntil = new Date(now);
    validUntil.setDate(validUntil.getDate() + 14); // 14 dias de validade
    ...
    this.logger.log(`[CouponsService] Cupom de indicação (indicador) ${coupon.code} emitido para ${userId} (referral ${referralId}).`);
    // Telemetria: referral_referrer_coupon_issued
    this.logger.log(`[TELEMETRY] referral_referrer_coupon_issued: { couponId: ${coupon.id}, userId: ${userId}, referralId: ${referralId} }`);
    return coupon;
  }
   ...
   */
  async resolveCoupon(code: string, userId: string, bookingData: {
    originalPrice?: number;
    clientId?: string;
    providerServiceId?: string;
    providerId?: string;
    scheduledDate?: string;
  }): Promise<{ coupon: any; eligibility: boolean; message: string; discountPreview?: number }> {
    const coupon = await this.prisma.coupon.findUnique({ where: { code: code.toUpperCase() } });
    if (!coupon) {
      this.logger.log(`[CouponsService] resolveCoupon: Cupom ${code} não encontrado para resolução.`);
      return { coupon: null, eligibility: false, message: 'Cupom não encontrado.' };
    }

    // Telemetria: coupon_resolved
    this.logger.log(`[TELEMETRY] coupon_resolved: { couponId: ${coupon.id}, code: ${coupon.code}, userId: ${userId} }`);
    ...
    return {
      coupon: coupon,
      eligibility: applicationResult.discountAmount > 0 || applicationResult.message === 'Cupom aplicado com sucesso!',
