import { Controller, Get, Req, UseGuards, HttpStatus, UnauthorizedException } from '@nestjs/common';
...
export class DashboardController {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Dados do dashboard do provedor.', type: DashboardDto })
  ...
  async getDashboardData(@Req() req: Request & { user: { userId: string, email: string, role: string } }): Promise<DashboardDto> {
    // Agora, 'req.user.userId' virá diretamente do JwtStrategy
    const userId = req.user.userId;
    if (!userId) {
      throw new UnauthorizedException('ID de usuário não encontrado no token JWT.');
    }
