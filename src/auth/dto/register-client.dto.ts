...
import { IsEmail, IsString, IsNotEmpty, MinLength, ValidateNested, IsOptional, Matches, Length } from 'class-validator'; // Adicionado Length
import { Type } from 'class-transformer';
import { CreateAddressDto } from '../../common/dto/create-address.dto'; // Assumindo o caminho correto

export class RegisterClientDto {
  @ApiProperty({ description: 'Endereço de e-mail do cliente', example: 'novo.cliente@example.com' })
  @IsEmail({}, { message: 'Formato de e-mail inválido.' })
  @IsNotEmpty({ message: 'O e-mail é obrigatório.' })
  email: string;

  @ApiProperty({ description: 'Senha do cliente (mínimo 8 caracteres, 1 maiúscula, 1 minúscula, 1 número, 1 caractere especial)', example: 'Senha@123' })
