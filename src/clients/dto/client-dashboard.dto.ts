...
class PopularServiceDto {
  ...
  name: string;
  @ApiProperty({ description: 'Número de vezes que foi agendado', example: 150 })
...
export class ClientDashboardDto {
  ...
  @ApiProperty({ description: 'Número de agendamentos concluídos', example: 10 })
  completedBookingsCount: number;

  @ApiPropertyOptional({ description: 'Próximo agendamento (opcional)', type: () => BookingEntity })
  nextBooking?: BookingEntity;

  @ApiPropertyOptional({ description: 'Lista de agendamentos recentes', type: () => [BookingEntity] })
  recentBookings?: BookingEntity[];

  @ApiPropertyOptional({ description: 'Lista de serviços populares', type: () => [PopularServiceDto] })
  popularServices?: PopularServiceDto[];

  @ApiPropertyOptional({ description: 'Avaliações pendentes para o cliente', type: () => [ReviewEntity] })
