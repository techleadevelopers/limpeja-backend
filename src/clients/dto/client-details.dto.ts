...
import { IsString, IsOptional, IsInt, IsNumber, ValidateNested, IsDateString } from 'class-validator';
...
export class ClientDetailsDto {
  ...
  @ApiPropertyOptional({ type: () => CreateAddressDto, description: 'Informações de endereço do cliente' })
  @IsOptional()
  @ValidateNested()
  @Type(() => CreateAddressDto)
  address?: CreateAddressDto | null;

  @ApiPropertyOptional({ description: 'Saldo da carteira do cliente', example: 150.75 })
  @IsOptional()
  @IsNumber()
  walletBalance?: number;

  @ApiPropertyOptional({ description: 'Número de agendamentos concluídos', example: 10 })
  @IsOptional()
  @IsInt()
  ordersCount?: number;
  ...
  @ApiPropertyOptional({ description: 'Ganhos totais no último mês (relevante para provedor, mas pode ser um campo geral de dashboard)', example: 500.00 })
  @IsOptional()
  @IsNumber()
  totalEarningsLastMonth?: number;

  @ApiPropertyOptional({ description: 'Número de agendamentos futuros', example: 2 })
  @IsOptional()
  @IsInt()
