...
export class GeocodingService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!this.googleMapsApiKey) {
      this.logger.warn('GOOGLE_MAPS_API_KEY não está configurada. A geocodificação de endereços pode não funcionar.');
    }
  }
  ...
  async geocodeAddress(address: string): Promise<GeocodeResponseDto | null> {
    if (!this.googleMapsApiKey) {
      this.logger.error('Não é possível geocodificar o endereço: GOOGLE_MAPS_API_KEY não está configurada.');
      throw new InternalServerErrorException('Serviço de geocodificação não configurado.');
    }
    ...
    try {
      ...
      if (response.data.status === 'OK' && response.data.results.length > 0) {
        ...
        this.logger.log(`Endereço geocodificado: Lat ${location.lat}, Lng ${location.lng}`);
        return {
        ...
        this.logger.error(`Erro na API de geocodificação para ${address}: ${response.data.status} - ${response.data.error_message || 'Erro desconhecido'}`);
        throw new InternalServerErrorException(`Falha ao geocodificar endereço: ${response.data.status}`);
      }
    } catch (error) {
      this.logger.error(`Erro ao chamar a API de geocodificação para ${address}: ${error.message}`);
      if (axios.isAxiosError(error) && error.response) {
        this.logger.error(`Detalhes do erro da API: ${JSON.stringify(error.response.data)}`);
      }
      throw new InternalServerErrorException('Erro ao se comunicar com o serviço de geocodificação.');
    }
  }
   ...
   */
  async getZoneByCoordinates(latitude: number, longitude: number): Promise<string> {
    this.logger.log(`Buscando zona para coordenadas: Lat ${latitude}, Lng ${longitude}`);

    // Exemplo de lógica simplificada:
    if (latitude > -23.6 && latitude < -23.5 && longitude > -46.7 && longitude < -46.6) {
      return 'ZONE_SAO_PAULO_CENTRO';
    } else if (latitude > -23.8 && latitude < -23.7 && longitude > -46.8 && longitude < -46.7) {
