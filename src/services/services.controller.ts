...
import { UserRole } from '@prisma/client'; // <-- Importar Service do Prisma para tipagem
import {
  ApiBearerAuth,
  ApiOperation,
  ApiResponse,
...
export class ServicesController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Criar um novo tipo de serviço (apenas para administradores)',
  })
  @ApiResponse({
    status: 201,
  ...
  async create(
    ...
    return new ServiceDetailsDto(service);
  }
  ...
  async findAll(): Promise<ServiceDetailsDto[]> {
    ...
    return services.map((service) => new ServiceDetailsDto(service));
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Atualizar um tipo de serviço (apenas para administradores)',
  })
  @ApiResponse({
    status: 200,
  ...
  async update(
    ...
    return new ServiceDetailsDto(updatedService);
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Deletar um tipo de serviço (apenas para administradores)',
  })
  @ApiResponse({
    status: 204,
    description: 'Tipo de serviço deletado com sucesso.',
  })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @ApiResponse({ status: 403, description: 'Acesso proibido.' })
  @ApiResponse({ status: 404, description: 'Tipo de serviço não encontrado.' })
