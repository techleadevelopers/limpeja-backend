...
export class LoyaltyService {
  ...
  async addPoints(dto: AddPointsDto): Promise<number> {
    ...
    if (type === LoyaltyTransactionType.REFERRAL && new Date().getMonth() === 7) {
      ...
      this.logger.log(`[LoyaltyService] Campanha ativa: pontos de indicação dobrados (${finalPoints}).`);
    }
    ...
    this.logger.log(`[LoyaltyService] ${finalPoints} pontos creditados ao usuário ${userId}. Saldo: ${loyalty.currentPoints}`);
  ...
  async redeemPoints(userId: string, redeemData: RedeemPointsDto): Promise<{
    ...
    if (!loyalty || loyalty.currentPoints < pointsToRedeem) {
      this.logger.warn(`[LoyaltyService] redeemPoints: Pontos insuficientes para resgate. Usuário ${userId} tem ${loyalty?.currentPoints || 0}, precisa de ${pointsToRedeem}.`);
    ...
    if (!reward || !reward.isActive) {
      this.logger.warn(`[LoyaltyService] redeemPoints: Recompensa ${rewardId} inválida ou inativa.`);
      throw new NotFoundException('Recompensa inválida ou inativa.');
    }
    if (reward.costPoints !== pointsToRedeem) {
      this.logger.warn(`[LoyaltyService] redeemPoints: Custo de pontos informado (${pointsToRedeem}) não corresponde ao da recompensa (${reward.costPoints}).`);
  ...
  private async getUserTier(userId: string): Promise<'BRONZE' | 'PRATA' | 'OURO' | 'PLATINA'> {
    ...
    this.logger.warn(`[LoyaltyService] getUserTier não implementado. Retornando BRONZE.`);
    return 'BRONZE';
  }
  ...
  private async getUserBookingStreak(userId: string): Promise<number> {
    ...
    this.logger.warn(`[LoyaltyService] getUserBookingStreak não implementado. Retornando 0.`);
  ...
  private async hasRecentGoodReview(userId: string): Promise<boolean> {
    ...
    // });
    // return !!recentReview;
    this.logger.warn(`[LoyaltyService] hasRecentGoodReview não implementado. Retornando false.`);
  ...
  async recalculateUserTiers(): Promise<void> {
    this.logger.log('[LoyaltyService] Iniciando recalculo de tiers dos usuários...');
    ...
    for (const loyalty of usersLoyalty) {
      ...
      this.logger.log(`[LoyaltyService] Usuário ${loyalty.userId}: ${totalPoints90d} pontos em 90 dias -> Tier ${newTier}`);
    }
    this.logger.log('[LoyaltyService] Recalculo de tiers concluído.');
  }
  ...
  async expireOldPoints(): Promise<void> {
    this.logger.log('[LoyaltyService] Iniciando expiração de pontos antigos...');
    ...
    this.logger.warn('[LoyaltyService] Lógica de expiração de pontos é complexa e requer modelagem de dados específica (ex: FIFO). Implementação conceitual.');
    this.logger.log('[LoyaltyService] Expiração de pontos concluída (conceitual).');
  }
}