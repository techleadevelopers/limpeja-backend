...
export class UserEntity implements Omit<PrismaUser, 'passwordHash'> {
  ...
  @ApiPropertyOptional({
    ...
    example: 'firebase-uid-exemplo',
  })
  firebaseUid: string | null;

  @ApiPropertyOptional({
    description: 'Token de notificação do Firebase Cloud Messaging',
  ...
  @ApiProperty({
    description: 'Status de verificação geral do usuário',
    example: true,
  })
  isVerified: boolean;

  @ApiPropertyOptional({
    description: 'Data e hora de agendamento de exclusão da conta (LGPD)',
  ...
  @ApiPropertyOptional({
    description: 'Código de indicação único do usuário',
    example: 'ABC123XYZ',
  })
  myReferralCode: string | null;

  @ApiProperty({
    description: 'Indica se a conta está ativa (soft delete)',
    example: true,
  })
