...
// Helper para gerar avalia��es em massa para um provedor (sem alterar l�gica principal)
async function generateBulkReviews(providerUser: any, clientUser: any, residentialService: Service, numReviews: number = 100) {
  console.log(`Gerando ${numReviews} avalia��es em massa para ${providerUser.fullName}...`);
  const ratingsCycle = [5, 4, 5, 4, 5, 3, 5, 4, 5, 5];
  const commentsCycle = [
    'Excelente servi�o! Muito profissional e atenciosa.',
    ...
    '�timo custo-benef�cio.',
    'Servi�o de qualidade.',
    'Recomendo sem hesitar.'
  ];

  // Encontrar ou criar ProviderService para o servi�o residencial (FIXED_PRICE, se existir; sen�o, qualquer)
  ...
  if (!providerService) {
    console.warn(`ProviderService n�o encontrado para ${providerUser.fullName}. Pulando gera��o de reviews.`);
    return;
  }

  let createdCount = 0;
  for (let i = 1; i <= numReviews; i++) {
    const pastDate = addDays(new Date(), - (i * 3 + Math.floor(Math.random() * 30))); // Datas passadas espa�adas e variadas
    ...
    const street = `Rua Avalia��o Fake ${i}`;
    ...
    const booking = await prisma.booking.upsert({
      ...
      update: {
        ...
        totalPrice: new Prisma.Decimal((150 + Math.floor(Math.random() * 100)).toFixed(2)), // Varia��o de pre�o
        addressId: address.id,
        notes: `Avalia��o em massa ${i} para ${providerUser.fullName}.`,
      },
      create: {
        id: bookingId,
        clientId: clientUser.client.id,
  ...
  console.log(`? ${createdCount} avalia��es geradas para ${providerUser.fullName}.`);
}
...
export async function main() {
  ...
  if (providerUser.provider) {
    ...
    console.log(`Transa��o de saldo inicial para ${providerUser.fullName} criada/atualizada. ID: ${initialTransaction.id}`);
  } else {
    ...
    create: {
      ...
      provider: {
        create: {
          ...
          acceptanceRate: 94,
          averageResponseTime: 25,
          bio: 'Dedica��o e cuidado em cada detalhe para um ambiente impec�vel.',
 ...
 console.log('Criando/Atualizando c�digo de indica��o...');
  ...
  console.log(
    `C�digo de Indica��o 'REF-TEST-001' para ${referrerUser.fullName} (indicando ${referredUser.fullName}) criado/atualizado.`,
  );

  // -----------------------------------------------------------------------------
  // 3) CAT�LOGO DE SERVI�OS
  console.log('Criando/Atualizando servi�os...');
  ...
  const servicesData = [
    ...
    { name: 'P�s-Obra', description: 'Limpeza detalhada ap�s reformas e constru��es.', icon: 'obra' },
    { name: 'Vidros', description: 'Limpeza especializada de janelas e superf�cies de vidro.', icon: 'vidro' },
    { name: 'Escrit�rio', description: 'Limpeza e organiza��o de espa�os de escrit�rio.', icon: 'escritorio' },
    { name: 'Estofados', description: 'Limpeza e higieniza��o de estofados.', icon: 'estofados' },
    { name: 'Passadoria', description: 'Servi�o de passar roupas.', icon: 'passadoria' },
    // Novos servi�os para BY_SIZE
    { name: 'Limpeza por M�', description: 'Limpeza cobrada por metro quadrado.', icon: 'area' }, // Pre�o por m2
    { name: 'Limpeza por C�modo', description: 'Limpeza cobrada por n�mero de c�modos.', icon: 'room' }, // Pre�o por c�modo
  ];
  ...
  for (const serviceData of servicesData) {
    ...
    console.log(`Servi�o '${service.name}' criado/atualizado.`);
  }

  const residentialService = createdServices['Residencial'];
  if (!residentialService) {
    throw new Error("Servi�o 'Residencial' n�o foi criado. Verifique o cat�logo e tente novamente.");
  }

  // --- OFERTAS DE SERVI�O POR PROVEDOR (COM TODOS OS TIPOS DE PRECIFICA��O) ---
  console.log('Criando/Atualizando ofertas de servi�o do provedor...');
  ...
  ensureHourlyServices(providerUser.provider!, [
    ...
    { serviceName: 'P�s-Obra', pricePerHour: hourlyRates.postConstruction, durationMinutes: 240, description: 'Limpeza p�s-obra com refor�o de equipe.' },
  ]);
  ...
  ensureHourlyServices(providerUser2.provider!, [
    ...
    { serviceName: 'Comercial', pricePerHour: 58, durationMinutes: 60, description: 'Cobran�a por hora para escrit�rios.' },
  ...
  ensureHourlyServices(providerUser3.provider!, [
    { serviceName: 'Residencial', pricePerHour: 62, durationMinutes: 200, description: 'Residencial com aten��o extra (at� 3h20m).' },
    { serviceName: 'Comercial', pricePerHour: 57, durationMinutes: 60, description: 'Limpeza comercial por hora com equipe fixa.' },
    { serviceName: 'P�s-Obra', pricePerHour: 80, durationMinutes: 240, description: 'P�s-obra com maior dura��o e checklist.' },
  ]);
  ...
  ensureHourlyServices(providerUser4.provider!, [
    ...
    { serviceName: 'P�s-Obra', pricePerHour: 76, durationMinutes: 240, description: 'P�s-obra com equipe reduzida e aten��o total.' },
  ]);
  ...
  const markBySizeNeedsReview = (provider: { id: string; fullName: string }, serviceName: string, note: string) => {
    ...
    console.log(`Servi�o ${serviceName} de ${provider.fullName} marcado para revis�o (BY_SIZE).`);
  };

  markBySizeNeedsReview(providerUser2.provider!, 'Limpeza por M�', 'Servi�o por �rea aguardando revis�o para hourly.');
  markBySizeNeedsReview(providerUser3.provider!, 'Limpeza por C�modo', 'Servi�o por c�modo aguardando revis�o para hourly.');
  ...
  for (const wd of weekdays) {
    ...
    if (existingAvailability) {
      await prisma.availability.update({
        ...
        data: {
          startTime: '09:00',
          endTime: '18:00', // MODIFICA��O: Estendendo o hor�rio dispon�vel para Carolina (de 12:00 para 18:00)
          isAvailable: true,
        },
      });
    } else {
  ...
  const additionalWeekdaysForCaroline = [2, 4, 6, 7]; // Ter�a, Quinta, S�bado, Domingo
  ...
  const weekdays2 = [2, 4]; // 2 = Ter�a, 4 = Quinta
  ...
  for (const wd of weekdays3) {
    ...
    if (existingAvailability) {
      await prisma.availability.update({
        ...
        data: {
          startTime: '09:00',
          endTime: '20:00', // MODIFICA��O: Estendendo o hor�rio dispon�vel para Joana (de 18:00 para 20:00)
          isAvailable: true,
        },
      });
    } else {
  ...
  const missionClient3Bookings = await prisma.mission.upsert({
    ...
    create: {
      ...
      title: 'Fa�a 3 reservas este m�s',
      description: 'Complete 3 agendamentos de servi�o em 30 dias para ganhar um cupom de R$10.',
      audience: MissionAudience.CLIENT,
      kind: MissionKind.COUNT_EVENT,
  ...
  const missionReview48h = await prisma.mission.upsert({
    ...
    update: {
      title: 'Avalie seu servi�o em at� 48h',
      audience: MissionAudience.CLIENT,
    ...
    create: {
      ...
      title: 'Avalie seu servi�o em at� 48h',
      description: 'Deixe uma avalia��o para seu servi�o em at� 48 horas e ganhe 200 pontos de fidelidade.',
      audience: MissionAudience.CLIENT,
      kind: MissionKind.WITHIN_WINDOW,
      eventName: 'review_submitted',
      targetValue: 1, // 1 avalia��o
      timeWindowDays: 2,
      rewardType: RewardType.POINTS,
      rewardValue: 200,
      isActive: true,
  ...
  const missionProviderAccept5 = await prisma.mission.upsert({
    ...
    update: {
      title: 'Aceite 5 solicita��es na semana',
  ...
  const missionProvider10Completed = await prisma.mission.upsert({
    ...
    update: {
      title: 'Conclua 10 servi�os no m�s',
      audience: MissionAudience.PROVIDER,
      kind: MissionKind.COUNT_EVENT,
      targetValue: 10,
      timeWindowDays: 30,
  ...
  const missionProviderHighRating = await prisma.mission.upsert({
    ...
    update: {
      title: 'Receba 10 avalia��es = 4.8 no m�s',
      audience: MissionAudience.PROVIDER,
      kind: MissionKind.COUNT_EVENT,
      targetValue: 10,
      timeWindowDays: 30,
    ...
    create: {
      ...
      title: 'Receba 10 avalia��es = 4.8 no m�s',
      description: 'Receba 10 avalia��es com nota = 4.8 em 30 dias (evento emitido no review).',
      audience: MissionAudience.PROVIDER,
      kind: MissionKind.COUNT_EVENT,
  ...
  console.log('Criando/Atualizando agendamentos e transa��es...');
  ...
  const pendingBooking = await prisma.booking.upsert({
    ...
    update: {
      ...
      notes: 'Solicita��o pendente para teste do dashboard.',
    },
    create: {
  ...
  const bookingJoana1 = await prisma.booking.upsert({
    ...
    update: {
      ...
      notes: 'Servi�o de limpeza para avalia��o da Joana (5 estrelas).',
  ...
  const bookingJoana2 = await prisma.booking.upsert({
    ...
    update: {
      ...
      status: BookingStatus.FINISHED,
      totalPrice: new Prisma.Decimal(190.00),
      notes: 'Servi�o de limpeza para avalia��o da Joana (4 estrelas).',
  ...
  for (const booking of [booking1, booking2, bookingJoana1, bookingJoana2]) {
    ...
    await prisma.transaction.upsert({
      ...
      update: {
        providerId: booking.providerId, // Usar o providerId do booking para a transa��o
        amount: booking.totalPrice,
        type: TransactionType.PAYMENT,
         status: 'COMPLETED',
        description: `Pagamento do servi�o ${booking.id}`,
        bookingId: booking.id,
      },
      create: {
        providerId: booking.providerId, // Usar o providerId do booking para a transa��o
    ...
    console.log(`Transa��o para Booking ${booking.id} criado/atualizado.`);
  }
  ...
  async function seedProviderLiveFlow() {
    console.log('Criando fluxo de bookings LIVE para testar iniciar/finalizar servi�o...');
    ...
    const liveBooking = await prisma.booking.upsert({
      ...
      create: {
        ...
        addressId: bookingAddressLive1.id, // ADICIONADO: Endere�o exclusivo
        notes: 'Booking confirmado para teste de in�cio de servi�o.',
      },
      update: {
        status: BookingStatus.CONFIRMED,
        scheduledDate: liveBookingDate, // Garantir consist�ncia na data
        scheduledTime: buildScheduledDateTime(liveBookingDate, '15:00'), // Garantir consistencia no hor�rio
        addressId: bookingAddressLive1.id, // Garantir consist�ncia no endere�o
      },
    });
    ...
    const inProgress = await prisma.booking.upsert({
      ...
      update: { 
        ...
        addressId: bookingAddressLive2.id, // Garantir consist�ncia no endere�o
      },
      create: {
        id: 'BKG-LIVE-2',
        clientId: referrerUser.client!.id,
        ...
        addressId: bookingAddressLive2.id, // ADICIONADO: Endere�o exclusivo
        notes: 'Servi�o em andamento para teste de finaliza��o.',
      },
    });
    ...
    const finished = await prisma.booking.upsert({
      ...
      update: { 
        status: BookingStatus.FINISHED,
        scheduledDate: finishedDate, // Garantir consist�ncia na data
        scheduledTime: buildScheduledDateTime(finishedDate, '11:00'), // Garantir consistencia no hor�rio
        addressId: bookingAddressLive3.id, // Mant�m o endere�o original (�nico)
      },
      create: {
        id: 'BKG-LIVE-3',
        clientId: referredUser.client!.id,
        ...
        notes: 'Servi�o finalizado para teste de hist�rico.',
      },
    });
    ...
    await prisma.transaction.upsert({
      ...
      create: {
        ...
        type: TransactionType.PAYMENT,
         status: 'COMPLETED',
        description: `Pagamento do servi�o finalizado ${finished.id}`,
        bookingId: finished.id,
      ...
      update: {
        ...
        type: TransactionType.PAYMENT,
         status: 'COMPLETED',
        description: `Pagamento do servi�o finalizado ${finished.id}`, // CORRIGIDO: servio -> servi�o
        bookingId: finished.id,
      },
    });
    console.log(`Transa��o de pagamento para Booking ${finished.id} criada/atualizada.`); // CORRIGIDO: Transao -> Transa��o
    ...
    await prisma.ledgerEntry.upsert({
      ...
      create: {
        ...
        bookingId: finished.id,
        note: `Ganhos do servi�o finalizado ${finished.id}`, // Usar note em vez de description // CORRIGIDO: servio -> servi�o
        createdAt: addDays(now, -2),
      },
      update: {
        ...
        bookingId: finished.id,
        note: `Ganhos do servi�o finalizado ${finished.id}`, // CORRIGIDO: servio -> servi�o
      },
    });
    console.log(`Ledger EARNING para Booking ${finished.id} criado/atualizado.`);

    // ---- NOTIFICA��ES PARA PROVIDER ---- (CORRIGIDO: Usar upsert com ID fixo para idempot�ncia) // CORRIGIDO: NOTIFICAES -> NOTIFICA��ES, idempotncia -> idempot�ncia

    // Notifica��o de servi�o iniciado (para BKG-LIVE-2) // CORRIGIDO: Notificao -> Notifica��o, servio -> servi�o
    await prisma.notification.upsert({
      where: { id: 'NTF-LIVE-1' },
      update: {
        userId: providerUser.id,
        type: 'booking_started',
        title: 'Servi�o iniciado', // CORRIGIDO: Servio -> Servi�o
        message: 'Voc� iniciou o servi�o agendado para 09:00 hoje.', // CORRIGIDO: Voc -> Voc�, servio -> servi�o
        isRead: false,
        targetUrl: '/(provider)/active-booking/BKG-LIVE-2',
        createdAt: now,
        idempotencyKey: `IDEMPOTENCY-NTF-LIVE-1`, // ADICIONADO: Para deduplica��o // CORRIGIDO: deduplicao -> deduplica��o
      },
      create: {
        id: 'NTF-LIVE-1',
        userId: providerUser.id,
    ...
    console.log(`Notifica��o de servi�o iniciado (BKG-LIVE-2) criada.`); // CORRIGIDO: Notificao -> Notifica��o, servio -> servi�o

    // Notifica��o de servi�o finalizado (para BKG-LIVE-3) // CORRIGIDO: Notificao -> Notifica��o, servio -> servi�o
    await prisma.notification.upsert({
      where: { id: 'NTF-LIVE-2' },
      update: {
        userId: providerUser.id,
        type: 'booking_completed',
        title: 'Servi�o finalizado', // CORRIGIDO: Servio -> Servi�o
        message: 'Voc� concluiu o servi�o de 11:00. Pagamento processado: R$240,00.', // CORRIGIDO: Voc -> Voc�, servio -> servi�o
        isRead: false,
        targetUrl: '/(provider)/earnings',
        createdAt: addDays(now, -2),
        idempotencyKey: `IDEMPOTENCY-NTF-LIVE-2`, // ADICIONADO: Para deduplica��o // CORRIGIDO: deduplicao -> deduplica��o
      },
      create: {
        id: 'NTF-LIVE-2',
        userId: providerUser.id,
    ...
    console.log(`Notifica��o de servi�o finalizado (BKG-LIVE-3) criada.`); // CORRIGIDO: Notificao -> Notifica��o, servio -> servi�o

    // Notifica��o de lembrete para servi�o futuro (BKG-LIVE-1) // CORRIGIDO: Notificao -> Notifica��o, servio -> servi�o
    await prisma.notification.upsert({
      where: { id: 'NTF-LIVE-3' },
      update: {
        userId: providerUser.id,
        type: 'booking_reminder',
        title: 'Lembrete de servi�o', // CORRIGIDO: servio -> servi�o
        message: 'Voc� tem um servi�o confirmado para hoje �s 15:00. Prepare-se!', // CORRIGIDO: Voc -> Voc�, servio -> servi�o, s -> �s
        isRead: false,
        targetUrl: '/(provider)/upcoming/BKG-LIVE-1',
        createdAt: now,
        idempotencyKey: `IDEMPOTENCY-NTF-LIVE-3`, // ADICIONADO: Para deduplica��o // CORRIGIDO: deduplicao -> deduplica��o
      },
      create: {
        id: 'NTF-LIVE-3',
        userId: providerUser.id,
    ...
    console.log(`Notifica��o de lembrete (BKG-LIVE-1) criada.`); // CORRIGIDO: Notificao -> Notifica��o
  ...
  console.log('Criando/Atualizando progresso de miss�es...'); // CORRIGIDO: misses -> miss�es
  ...
  console.log(`Transa��o de pontos para ${referrerUser.fullName} (servi�o conclu�do) criada/atualizada.`); // CORRIGIDO: Transao -> Transa��o, servio -> servi�o

  // -----------------------------------------------------------------------------
  // 9) NOTIFICA��ES // CORRIGIDO: NOTIFICAES -> NOTIFICA��ES
  console.log('Criando/Atualizando notifica��es...'); // CORRIGIDO: notificaes -> notifica��es

  console.log(`Notifica��o de boas-vindas para ${referredUser.fullName} criada/atualizada.`); // CORRIGIDO: Notificao -> Notifica��o

    // -----------------------------------------------------------------------------
  // 10) AVALIA��ES (REVIEWS) // CORRIGIDO: AVALIAES -> AVALIA��ES
  console.log('Criando/Atualizando avalia��es...'); // CORRIGIDO: avaliaes -> avalia��es

  await prisma.review.upsert({
    where: { bookingId: booking2.id },
    update: {
      rating: 5,
      comment: 'Servi�o excelente! Provedora muito profissional e atenciosa.', // CORRIGIDO: Servio -> Servi�o
      clientId: referrerUser.client!.id,
      providerId: providerUser.provider!.id,
    },
    create: {
  ...
  console.log(`Avalia��o para Booking ${booking2.id} criada/atualizada.`); // CORRIGIDO: Avaliao -> Avalia��o

  // NOVO: Avalia��es para Joana // CORRIGIDO: Avaliaes -> Avalia��es
  await prisma.review.upsert({
  ...
  console.log(`Avalia��o para Booking ${bookingJoana1.id} (Joana) criada/atualizada.`); // CORRIGIDO: Avaliao -> Avalia��o

  await prisma.review.upsert({
    where: { bookingId: bookingJoana2.id },
    update: {
      rating: 4,
      comment: 'Bom servi�o, chegou no hor�rio e fez um bom trabalho. Fiquei satisfeita.', // CORRIGIDO: servio -> servi�o, horrio -> hor�rio
      clientId: clientUserJoanaReviewer.client!.id,
      providerId: providerUser3.provider!.id,
    },
    create: {
  ...
  console.log(`Avalia��o para Booking ${bookingJoana2.id} (Joana) criada/atualizada.`); // CORRIGIDO: Avaliao -> Avalia��o

  // -----------------------------------------------------------------------------
  // GERA��O EM MASSA DE 100 AVALIA��ES POR PRESTADOR (usando o servi�o residencial e reviewers dedicados) // CORRIGIDO: GERAO -> GERA��O, AVALIAES -> AVALIA��ES, servio -> servi�o
  // -----------------------------------------------------------------------------
  console.log('Gerando avalia��es em massa para prestadores...'); // CORRIGIDO: avaliaes -> avalia��es
  ...
  await prisma.fAQItem.upsert({
    where: { question: 'Como fa�o para agendar um servi�o?' }, // CORRIGIDO: fao -> fa�o, servio -> servi�o
    update: { answer: 'Voc� pode agendar um servi�o atrav�s da tela "Explorar", escolhendo a categoria e o provedor desejado.' }, // CORRIGIDO: Voc -> Voc�, servio -> servi�o, atravs -> atrav�s
    create: {
      question: 'Como fa�o para agendar um servi�o?', // CORRIGIDO: fao -> fa�o, servio -> servi�o
      answer: 'Voc� pode agendar um servi�o atrav�s da tela "Explorar", escolhendo a categoria e o provedor desejado.', // CORRIGIDO: Voc -> Voc�, servio -> servi�o, atravs -> atrav�s
      category: 'Geral',
      order: 1,
    },
  });
  console.log(`FAQ 'Como fa�o para agendar um servi�o?' criado/atualizado.`); // CORRIGIDO: fao -> fa�o, servio -> servi�o
  ...
  if (existingOffer) {
    await prisma.offer.update({
      ...
      data: {
        description: '10% de desconto em todos os servi�os de limpeza residencial!', // CORRIGIDO: servios -> servi�os
  ...
  await prisma.reward.upsert({
    ...
    update: {
      description: 'Resgate um cupom de R$25 para usar em qualquer servi�o.', // CORRIGIDO: servio -> servi�o
      costPoints: 500,
  ...
  // PricingRule
  console.log('Criando/Atualizando regras de precifica��o...'); // CORRIGIDO: precificao -> precifica��o
  ...
  console.log(`Regra de precifica��o 'PRICING-RULE-1' criada/atualizada.`); // CORRIGIDO: precificao -> precifica��o
  ...
  const dummySupportTicketForDispute = await prisma.supportTicket.upsert({
    ...
    create: {
      ...
      subject: 'Dummy Ticket for Dispute Message',
      category: SupportTicketCategory.OTHER,
      description: 'Este � um ticket dummy para satisfazer a rela��o de DisputeMessage.', // CORRIGIDO:  -> �, relao -> rela��o
      status: SupportTicketStatus.CLOSED,
    },
  });
  ...
  console.log(`Consentimento para ${referrerUser.fullName} (Termos de Servi�o) criado/atualizado.`); // CORRIGIDO: Servio -> Servi�o
  ...
  await prisma.incident.upsert({
    ...
    create: {
      ...
      bookingId: booking2.id,
      type: IncidentType.DAMAGE,
      description: 'Um objeto foi danificado durante o servi�o.', // CORRIGIDO: servio -> servi�o
      attachments: [],
      status: IncidentStatus.PENDING_REVIEW,
    },
  });
  ...
  // GuaranteeClaim
  console.log('Criando/Atualizando solicita��o de garantia...'); // CORRIGIDO: solicitao -> solicita��o
  ...
  console.log(`Solicita��o de Garantia para Booking ${booking2.id} criada/atualizada.`); // CORRIGIDO: Solicitao -> Solicita��o

  console.log('Seed completo com fluxo LIVE e avalia��es em massa! ?'); // CORRIGIDO: avaliaes -> avalia��es
}
